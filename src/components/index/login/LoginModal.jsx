  ...
  return (
    <div className={style.login} onClick={() => { switchTo("none") }}>
      <div className={style["login-content"]} onClick={(e) => e.stopPropagation()}> {/*阻止事件冒泡*/}
        ...
        <div>
          <span onClick={() => switchTo("forget")} style={{ cursor: "pointer" }} className={style.forget}>忘記密碼？</span> |
          <span onClick={() => switchTo("register")} style={{ cursor: "pointer" }} className={style.now}>立即註冊</span>
        </div>
      </div>
    </div>
  );
